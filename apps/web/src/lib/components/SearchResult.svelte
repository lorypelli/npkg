<script lang="ts">
	import type { User } from '@npkg/types';
...
<a href="/{name}" {onclick}>
	...
	>
		...
		{#if description}
			<div
				class={twMerge('overflow-y-hidden pb-1 leading-3 whitespace-normal', small && 'max-h-20')}
			>
				<span class="text-sm break-words">{description}</span>
			</div>
		{/if}
		<div class="flex flex-wrap items-center gap-2">
			<span class="rounded-xl border-2 border-black p-1 text-center dark:border-white"
				>{version}
				<span class="rounded-md border-2 border-black px-1 dark:border-white"
