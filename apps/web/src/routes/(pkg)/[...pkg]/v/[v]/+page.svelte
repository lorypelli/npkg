...
<svelte:head>
	...
	{#if data.author}
		{#if data.author.username || data.author.name}
			<meta name="author" content={data.author.username || data.author.name} />
		{/if}
	{/if}
	{#if data.keywords}
...
>
	<div class="flex flex-col px-2 md:w-1/4 md:overflow-auto">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2">
			<div class="flex flex-col items-center">
				...
				<a href="/{data.name}/v" class="text-sm hover:underline">Click Here...</a>
			</div>
			<div class="flex flex-col items-center">
			...
			<div class="flex flex-col items-center">
				...
				<a href="#scripts" class="text-sm hover:underline">Click Here...</a>
			</div>
			<div class="flex flex-col items-center">
				<span class="text-lg font-bold">Code:</span>
				<a href="/{data.name}/v/{data.version}/code" class="text-sm hover:underline"
					>Click Here...</a
				>
			</div>
		</div>
		{#if data.keywords && data.keywords.length > 0}
			...
			<div class="grid grid-cols-2 gap-2">
				<Keywords keywords={data.keywords} limit={data.keywords.length} />
			</div>
		{/if}
	</div>
	...
	<div class="flex flex-col px-2 md:w-1/4 md:overflow-auto">
			...
			{#if data.homepage}
				<div class="flex flex-col">
					...
					<a href={data.homepage} target="_blank" class="hover:underline">
						<span class="break-words">{data.homepage.replace(/(http?s:\/\/)|(\/$)/g, '')}</span>
					</a>
				</div>
			{/if}
			{#if data.license}
			...
			{#if data.dist.unpackedSize}
				<div class="flex flex-col">
					...
					<div class="grid grid-cols-2">
						<span>{(data.dist.unpackedSize / (1000 * 1000)).toFixed(2)} MB</span>
						<span>{(data.dist.unpackedSize / (1024 * 1024)).toFixed(2)} MiB</span>
					</div>
				</div>
			{/if}
		</div>
		...
		{#if data.contributors && data.contributors.length > 0}
			<span class="font-extrabold">Contributors ({data.contributors.length}):</span>
			{#each data.contributors as c}
				{#if c.username || c.name}
					<div class="flex items-center justify-center space-y-1 gap-x-1">
						<Avatar email={c.email} />
						{#if c.url}
							<a href={fixURL(c.url)} target="_blank" class="hover:underline">
				...
				{:else}
					<span class="font-bold text-red-600">An error occurred!</span>
				{/if}
			{/each}
		{/if}
		{#if data.maintainers && data.maintainers.length > 0}
			<span class="font-extrabold">Maintainers ({data.maintainers.length}):</span>
...
<Dependencies dependencies={data.dependencies} devDependencies={data.devDependencies} />
